...
// Refer to https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-device-sdk-c-intro for more information on Azure IoT SDK for C
...
bool AzureIoT_SetupClient(void)
{
	...
	if (IoTHubDeviceClient_LL_SetConnectionStatusCallback(iothubClientHandle, hubConnectionStatusCallback, NULL) != IOTHUB_CLIENT_OK) {
		LogMessage("ERROR: failure setting callback\n");
		return false;
	}

	// Set retry policy for the connection to the IoT Hub.
	if (IoTHubDeviceClient_LL_SetRetryPolicy(iothubClientHandle, IOTHUB_CLIENT_RETRY_INTERVAL, retryTimeoutSeconds) != IOTHUB_CLIENT_OK) {
...
/// <summary>
///     Sets the function to be invoked whenever the Device Twin properties have been delivered to the IoT Hub.
/// </summary>
/// <param name="callback">The function pointer to the callback function.</param>
void AzureIoT_SetDeviceTwinDeliveryConfirmationCallback(DeviceTwinDeliveryConfirmationFnType callback)
{
...
/// <summary>
///     Creates and enqueues a report containing the name and value pair of a Device Twin reported property.
...
void AzureIoT_TwinReportState(const char* propertyName, size_t propertyValue)
{
	...
	if (reportedPropertiesString == NULL) {
		LogMessage("ERROR: could not serialize the JSON payload to string for Device "			"Twin reporting.\n");
		goto cleanup;
	}
...
/// </summary>
void AzureIoT_TwinReportStateJson(char* reportedPropertiesString, size_t reportedPropertiesSize)
{
	...
	if (iothubClientHandle == NULL) {
		...
		if (reportedPropertiesString != NULL) {
			if (IoTHubDeviceClient_LL_SendReportedState(iothubClientHandle, (unsigned char*)reportedPropertiesString, reportedPropertiesSize, reportStatusCallback, 0) != IOTHUB_CLIENT_OK) {
