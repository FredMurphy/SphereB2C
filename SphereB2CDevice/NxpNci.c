...
bool NxpNci_WaitForDiscoveryNotificationTimeout(NxpNci_RfIntf_t *pRfIntf, uint16_t timeout)
{
...
wait:
    ...
    do
    {
        if(NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, timeout) == NXPNCI_ERROR) return NXPNCI_ERROR;
...
bool NxpNci_WaitForDiscoveryNotification(NxpNci_RfIntf_t* pRfIntf) {
	return NxpNci_WaitForDiscoveryNotificationTimeout(pRfIntf, TIMEOUT_INFINITE);
}
